...
global.getAllBalances = function(callback){
...
commands['pay'] = function(data){
	...
	var to = parts[0];
	var amount = parts[1].Replace(',');
	transfer(data.name,to, amount, function(pass){
		if(data.private && getUUID(to) && client.username.toLowerCase() != to.toLowerCase()){
			chat.pm(to,data.name + ' has just payed you $' + amount + '.');
		}
		data.respond(data.name + ' -> ' + parts[0] + ': ' + (pass ? 'passed' : 'failed'));
	});
}

commands['baltop'] = topbal;
commands['topbal'] = topbal;

function topbal(data){
	getAllBalances(function(players){
